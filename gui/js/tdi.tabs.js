(
	function($){
		...
		$.fn.dynatabs = function(options){
			...
			$.fn.closeTab = function(event){
				...
				if(event.data.key != null && event.data.tab != null)
				{
					...
					if(canDelete)
					{
						...
						{
							eel.willBeClosed(document.querySelector(`a[href="${event.data.key}"]`).innerHTML) // added by nbsas 20201028T002520TZ0300
			...
			$.getActiveTab = function(){ //added by nbsas
				return ctKey
			}
			...
			$.fn.addNewTab = function(defaults, tabBody){
				...
				if(defaults.type === 'html')
				{
					if(defaults.html != null && defaults.html.length > 0)
					{
						...
						//create data div
						var div = $("<div />");
						// var len = tabs.find("li").length + 1; // This variable was not providing a proper way too keep tracks of new tabs and caused overwritten tab contents, so I, nbsas replaced this with tabCounter_nbsas. The new counter's data is controlled by index.html
						$(a).attr('href', '#tabview' + '_' + settings.tabBodyID + tabCounter_nbsas);
						$(a).text(defaults.tabTitle);
						$(li).append(a);
						$(div).attr('id', 'tabview' + '_' + settings.tabBodyID + tabCounter_nbsas);
						$(div).attr('style', 'height:97%;'); // Added by nbsas
						
						// $(div).attr('style', 'height:940px;'); // Added by nbsas
